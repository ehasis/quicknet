...
Os seguintes itens estão explicitamente fora do escopo do MVP e serão considerados para versões futuras:

- **Autocomplete / IntelliSense** — popup de sugestões durante a digitação.
- **Syntax highlighting** — fora do escopo do MVP; será introduzido de forma básica a partir da v1.2.
- **Compartilhamento de contexto entre execuções** — cada input é compilado e executado isoladamente (sem estado persistente entre linhas).
- **Meta-comandos** (`/clear`, `/reset`, `/help`, `/load`).
- **Modo CLI / Terminal** — apenas GUI via Avalonia no MVP.
- **Suporte a Linux e macOS** — Windows apenas na v1.0.
- **Debugger integrado**.
...
│                   Avalonia UI                        │
│  ┌─────────────┐  ┌──────────────┐  ┌────────────┐ │
│  │ Conversation │  │   Language   │  │   Input    │ │
│  │   Panel      │  │   Selector   │  │   Field    │ │
│  └─────────────┘  └──────────────┘  └────────────┘ │
└──────────────────────┬──────────────────────────────┘
                       │
┌──────────────────────▼──────────────────────────────┐
...
Input do Usuário
       ...
       ▼
┌──────────────┐
│  1. Parse    │  Detecta linguagem ativa, anexa ao template
└──────┬───────┘
       ▼
┌──────────────┐
│  2. Compile  │  Roslyn CSharpCompilation / VisualBasicCompilation
└──────┬───────┘
       ▼
┌──────────────┐     ┌─────────────────┐
│  3. Execute  │────▶│ Sucesso: captura │
│  (Reflection)│     │ resultado via    │
└──────────────┘     │ reflection       │
       │             └─────────────────┘
       │             ┌─────────────────┐
       └────────────▶│ Falha: exibe     │
       ...
       ▼
┌──────────────┐
│  4. Display  │  Formata resultado (ToString / JSON / exception)
└──────────────┘
```
...
- [ ] Configuração de timeout de execução
